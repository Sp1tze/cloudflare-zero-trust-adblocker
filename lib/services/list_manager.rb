...
module Services
  class ListManager
    ...
    def self.fetch_existing_lists
      ...
      url = 'https://api.cloudflare.com/client/v4/accounts/' + account_id + '/gateway/rules/lists'
      result = API::CloudflareAPI.api_call(:get, url)
    ...
    def self.create_list(name, description, domains)
      ...
      url = 'https://api.cloudflare.com/client/v4/accounts/' + account_id + '/gateway/rules/lists'
      API::CloudflareAPI.api_call(:post, url, body)
    end

    def self.delete_list(list_id)
      ::Utils::Log.logger.info("Deleting list with ID #{list_id}...")
      url = 'https://api.cloudflare.com/client/v4/accounts/' + account_id + '/gateway/rules/lists/' + list_id.to_s
      API::CloudflareAPI.api_call(:delete, url)
    end
  end
end
